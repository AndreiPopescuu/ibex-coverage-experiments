...
def cheat_sheet_text(target_csrs=None) -> str:
    ...
    lines.append("")
    lines.append("EXCEPTION CAUSES — mcause value set by hardware for each exception type:")
    lines.append("  ECALL from M-mode  → mcause=11 (0xB), mepc=PC of ECALL,  mtval=0")
    lines.append("  EBREAK             → mcause=3,         mepc=PC of EBREAK, mtval=0")
    lines.append("  Illegal instr      → mcause=2,         mepc=PC of instr,  mtval=instr word")
    lines.append("  Instr misaligned   → mcause=0,         mepc=PC,           mtval=target addr")
    lines.append("  Load misaligned    → mcause=4,         mepc=PC of load,   mtval=bad address")
    lines.append("  Store misaligned   → mcause=6,         mepc=PC of store,  mtval=bad address")
    lines.append("  To get diverse mcause/mtval bits: mix ECALL (cause=11=0b1011),")
    lines.append("  misaligned LW at x20+1 (cause=4, mtval=0x10001), EBREAK (cause=3=0b11).")
    lines.append("")
    lines.append("CSR STATE AFTER HARNESS PROLOGUE (before your program starts):")
    lines.append("  mtvec   = 0x00200000  (direct mode, handler pre-installed)")
    lines.append("  mstatus = 0x1800      (MPP=11, running in M-mode)")
    lines.append("  mcycle  ≈ 100         (small value from harness setup)")
    lines.append("  minstret≈ 50          (small value)")
    lines.append("  mepc    = 0           (no exception taken yet)")
    lines.append("  mcause  = 0           (no exception yet)")
    lines.append("  mtval   = 0           (no exception yet)")
    lines.append("  mcountinhibit = 0     (counters running; bits 0=CY 2=IR are writable)")
    lines.append("")
    lines.append("IBEX HARDWARE CONFIG — signals that are HARDWIRED TO 0 in this build.")
    lines.append("  Do NOT write programs targeting these — they cannot toggle:")
    lines.append("  DbgTriggerEn=0  → tmatch_*, debug_mode*, debug_cause*,")
    lines.append("                    debug_ebreak*, debug_single_step*, illegal_csr_dbg")
    lines.append("  SecureIbex=0    → *_err (mstatus_err, mtvec_err, csr_shadow_err_o),")
    lines.append("                    cpuctrlsts_part_*, data_ind_timing*, pc_mismatch_alert")
    lines.append("  MHPMCounterNum=0→ mcountinhibit[3-31], mhpmcounter*, mhpmevent*")
    lines.append("  IRQ inputs=0    → irq_*, mip, nmi_*")
    lines.append("  RV32B=None      → imd_val_q_ex[1][*], clmul_result, pack_result, etc.")
    lines.append("  RISC-V arch     → mepc[0]=0 always (alignment), mtvec[1]=0 always (MODE),")
    lines.append("                    pc[0]=0 always — these bits CANNOT be toggled")
    lines.append("")
