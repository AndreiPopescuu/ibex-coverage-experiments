"""build_corpus.py — rulează env L9 și salvează programele care aduc bins noi.

Usage:
    python build_corpus.py --episodes 300 --out corpus_l9.json
    python build_corpus.py --episodes 300 --model l9_v2_checkpoint_model.zip --out corpus_l9.json
"""
...
from codec_l9 import emit_program
from env_l9_v2 import IbexL9V2Env, run_program
sys.path.insert(0, str(THIS.parent / "level8_ops"))
...
def main():
    ...
    ap.add_argument("--out",      default="corpus_l9.json")
    ap.add_argument("--seed",     type=int, default=42)
    args = ap.parse_args()

    rng = np.random.default_rng(args.seed)
    env = IbexL9V2Env(episode_steps=args.steps)
    ...
    for ep in range(args.episodes):
        ...
        # hits-urile acestui episod, normalizate
        prefix = "\x01page\x02v_toggle/"
        ep_hits_raw = {k for k, v in env._cum_hits.items()
                       if prefix in ("\x01" + k)} if hasattr(env, '_cum_hits') else set()

        # recalculam new_hits fata de corpus-ul nostru acumulat
        words = emit_program(env._actions)

        # rulam direct ca sa obtinem hits episodului izolat
        ...
        cum_pct = 100.0 * len(cum_hits) / max(summary.by_kind["toggle"][1], 1)
        print(f"  ep {ep:>4}: +{len(new_hits):>4} bins noi  |  cum {cum_pct:.2f}%  |  "
              f"program salvat: {'DA' if new_hits else 'nu'}")
