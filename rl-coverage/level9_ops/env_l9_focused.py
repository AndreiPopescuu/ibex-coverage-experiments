"""env_l9_focused.py — L9 env focalizat.

Față de env_l9_v2:
  - Op set redus la 30 ops (ibex_controller + CSR paths)
  - csr_bucket ca dimensiune explicită în action space
    → agentul controlează direct ce CSR accesează
  - Reward = episode_base + toggle_shaped + 0.3 * branch_new
  - Urmărire branch hits (ca în L10 focused)
"""
...
PROGRAM_JSON = "/tmp/rl_l9_focused.json"

sys.path.insert(0, str(L5))
import cov_parser

from codec_l9_focused import N_OPS, IMM_BUCKETS, N_CSR_BUCKETS, emit_program
...
class IbexL9FocusedEnv(gym.Env):
