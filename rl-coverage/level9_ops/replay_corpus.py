"""replay_corpus.py — replays un corpus salvat și raportează coverage-ul final.

Regression test: dacă coverage-ul replayat == coverage-ul din corpus, testul trece.

Usage:
    python replay_corpus.py --corpus corpus_l9.json
    python replay_corpus.py --corpus corpus_l9.json --expected 71.5
"""
...
PROGRAM_JSON = "/tmp/rl_replay.json"
...
def run_words(words: list[int]):
    """Rulează un program dat ca listă de cuvinte 32-bit pe Verilator."""
...
def main():
    ...
    ap.add_argument("--corpus",   required=True, help="Fișier JSON generat de build_corpus.py")
    ap.add_argument("--expected", type=float, default=None,
                    help="Coverage așteptat (%). Dacă e specificat, testul eșuează dacă nu e atins.")
    args = ap.parse_args()
    ...
    print(f"Corpus: {total_prg} programe | coverage așteptat din corpus: {corpus['final_cum_pct']}%\n")
    ...
    if args.expected is not None:
        if final_pct >= args.expected:
            print(f"\n[PASS] Coverage {final_pct:.2f}% >= {args.expected}% așteptat")
            sys.exit(0)
        else:
            print(f"\n[FAIL] Coverage {final_pct:.2f}% < {args.expected}% așteptat")
            sys.exit(1)
