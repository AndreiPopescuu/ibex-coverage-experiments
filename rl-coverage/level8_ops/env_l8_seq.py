"""Level 8 sequential env for PPO.

Key differences from env_l8_rich:
  - episode_steps = 256 (not 1024) → 4x more gradient updates at same wall time
  - observation includes last 4 emitted op indices (normalised 0..1) so PPO
    can learn sequential patterns like JAL → JALR or LUI → MUL

Observation (35 dims):
  [0]    step_idx / episode_steps
  [1]    n_episodes / 300
  [2]    overall cumulative toggle fraction
  [3:31] per-module toggle-covered fraction (28 modules)
  [31:35] last 4 op indices / (N_OPS-1), oldest first (0 before first action)
"""
...
PROGRAM_JSON = "/tmp/rl_l8_seq_program.json"
...
HISTORY_LEN = 4
OBS_DIM = 3 + N_MODULES + HISTORY_LEN
...
class IbexL8SeqEnv(gym.Env):
    ...
    def __init__(self, episode_steps: int = 256, seed: int | None = None,
        ...
        self._recent_ops: deque = deque([0.0] * HISTORY_LEN, maxlen=HISTORY_LEN)
    ...
    def _obs(self) -> np.ndarray:
        ...
        history = np.array(list(self._recent_ops), dtype=np.float32)
        return np.concatenate([head, self._module_cov, history])

    def reset(self, *, seed=None, options=None):
        self._actions.clear()
        self._step_idx = 0
        self._recent_ops = deque([0.0] * HISTORY_LEN, maxlen=HISTORY_LEN)
        return self._obs(), {}

    def step(self, action):
        op_i = int(action[0])
        self._actions.append(tuple(int(x) for x in action))
        self._recent_ops.append(op_i / (N_OPS - 1))
        self._step_idx += 1
