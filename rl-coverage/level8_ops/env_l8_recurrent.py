"""env_l8_recurrent.py — L8 env pentru RecurrentPPO (LSTM policy).

Față de env_l8_dynamic.py:
  - Folosește codec_l8_2 (72 ops: L8 + MRET + WFI)
  - Observation space extins cu ultimele N_HIST=4 op-uri executate (normalizate)
    → LSTM primește context explicit pe termen scurt, reduce sarcina pe hidden state
  - Reward shaping dinamic identic (weight = 1 / coverage_fraction)

Observation layout (14 features total):
  [step_frac, cum_cov, mod_0..mod_7, last_op_0..last_op_3]
   0          1        2..9          10..13
"""
...
PROGRAM_JSON = "/tmp/rl_l8_recurrent.json"

sys.path.insert(0, str(L5))
import cov_parser

from codec_l8_2 import N_OPS, IMM_BUCKETS, emit_program
...
N_HIST = 4  # câte acțiuni recente includem în obs
N_OBS  = 2 + len(TRACKED_MODULES) + N_HIST
...
class IbexL8RecurrentEnv(gym.Env):
    ...
    def __init__(self, episode_steps: int = 256, seed: int | None = None,
        ...
        # Ultimele N_HIST op-uri (normalizate 0-1), inițializate cu 0
        self._last_ops: deque = deque([0.0] * N_HIST, maxlen=N_HIST)
    ...
    def _obs(self) -> np.ndarray:
        ...
        last_ops = list(self._last_ops)  # deja normalizate la inserare
        return np.array([step_frac, cum_cov] + mod_fracs + last_ops, dtype=np.float32)

    def reset(self, *, seed=None, options=None):
        self._actions.clear()
        self._step_idx = 0
        self._last_ops = deque([0.0] * N_HIST, maxlen=N_HIST)
        return self._obs(), {}

    def step(self, action):
        op_i = int(action[0])

        # Actualizează istoricul de acțiuni ÎNAINTE de a calcula obs
        # → obs returnat va include acțiunea curentă
        self._last_ops.append(op_i / N_OPS)
