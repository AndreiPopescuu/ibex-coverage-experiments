"""train_l8_2_ppo.py — PPO cu codec L8.2 (72 ops: +MRET +WFI).

Față de train_l8_dynamic_ppo.py:
  - Codec L8.2: adaugă MRET și WFI la cei 70 ops existenți
  - MRET exercită ibex_controller FSM return-from-trap + ibex_cs_registers
  - Restul identic: dynamic weights, 1024 steps, 300 ep

Usage:
    python train_l8_2_ppo.py
    python train_l8_2_ppo.py --episodes 300 --hits l8_pipeline_hits.pkl
"""
...
from env_l8_2 import IbexL8_2Env, TRACKED_MODULES
...
def main():
    ...
    ap.add_argument("--out",      default="l8_2_ppo_curve.npz")
    args = ap.parse_args()

    print("=" * 64)
    print("L8.2 PPO — codec 72 ops (+MRET +WFI), weights dinamice")
    print("=" * 64)
    ...
    print(f"  Codec:         L8.2 (72 ops: L8 + MRET + WFI)")
    ...
    if cb.history:
        ...
        print(f"\nRezultate finale:")
        print(f"  Baseline:              {baseline_pct:.2f}%")
        print(f"  L8.2 PPO best:         {best:.2f}%")
        print(f"  L8.2 PPO final:        {final:.2f}%")
